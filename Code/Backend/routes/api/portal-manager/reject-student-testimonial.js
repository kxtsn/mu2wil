...
console.log("Reject Student Testimonial")
router.post('/', isLoggedIn, async function (req, res, next) {
    ...
    if (req.userData["role"] == 1 || req.userData["role"] == 2) {
        ...
        try {
            ...
            const mresult = await conn.query(`UPDATE student_testimonial SET Status = 'Rejected' WHERE Student_Testimonial_ID = ${pool.escape(req.body.testimonialId)}`)
            ...
            res.status(201).send({
                msg: 'Student Testimonial Rejected'
            });
