...
console.log("Approve Employer Testimonial")
router.post('/', isLoggedIn, async function (req, res, next) {
    ...
    if (req.userData["role"] == 1 || req.userData["role"] == 2) {
        ...
        try {
            ...
            const mresult = await conn.query(`UPDATE employer_testimonial SET Status = 'Approved' WHERE Employer_Testimonial_ID = ${pool.escape(req.body.testimonialId)}`)
            ...
            res.status(201).send({
                msg: 'Employer Testimonial Approved'
            });
