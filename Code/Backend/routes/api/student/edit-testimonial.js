...
router.post('/', isLoggedIn, async function (req, res, next) {
    ...
    if (req.userData["role"] == 3) {
        ...
        try {
            ...
            const mresult = await conn.query(`UPDATE student_testimonial SET Comment =  ${pool.escape(req.body.comment)}, File =  ${pool.escape(req.body.blob)}, Status = 'Pending' WHERE Student_Testimonial_ID = ${pool.escape(req.body.testimonialId)}`)
