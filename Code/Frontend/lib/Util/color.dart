import 'package:flutter/material.dart';

const whiteColor = Color(0XFFFFFFFF); //white
const kBackGroundColor = Color(0XFFfdfdfc); //off white
const primaryColor = Color(0xFFe03753); //mu red
const blackColor = Color(0xff000000); //black
const darkGreyColor = Color(0xffaaa2a3); //dark grey
const greyColor = Color(0xffd4cfd0); //grey
const greenColor = Color(0xff3CB043); //green
const redColor = Color(0xffBF0A0A); //red

const evenColor = Color(0xFFF3F3F3);
const tabletext = Color(0xFF000000);
